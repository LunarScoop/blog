...
CoT3DRef 将三维视觉定位从直接选择最终目标重新表述为预测一条由锚点和目标组成的对象序列。给定三维场景和自然语言描述，模型首先识别描述中出现的对象，随后由 Pathway Head 预测对象的逻辑定位顺序，再使用 Parallel Referring Head 产生初始位置，最后由 CoT Decoder 按照该顺序细化锚点和目标的位置。

论文中的 Chain-of-Thought 不是自然语言推理文本，而是由对象类别和候选物体位置构成的结构化中间结果。它的主要作用是增加中间监督、显式建模对象之间的定位依赖，并为错误诊断提供可检查的过程输出。

论文在 Nr3D、Sr3D 和 ScanRefer 上进行评估，并将模块集成到 LAR、SAT、MVT 和 ViL 等已有架构中。实验结果表明，链式定位在不同基线和数据规模下都能带来稳定收益；同时，自动伪标签质量和三维候选物体生成质量仍然构成方法的性能上限。
...
论文所处的研究脉络主要包括三类方法。第一类方法使用场景图或图神经网络，将候选物体作为节点、空间关系作为边；第二类方法使用 Transformer 或跨模态注意力，对语言 token 和物体特征进行联合建模；第三类方法利用多视角图像、二维语义蒸馏或相对距离与方向信息，提高三维特征的表达能力。

这些方法通常能够在特征层面建模对象关系，但最终仍将任务归结为一次性的目标选择。与之相比，CoT3DRef 关注的是**预测过程的显式结构**：它不仅预测目标，还预测描述中的锚点、锚点顺序以及锚点位置，并将这些中间变量传入后续解码器。

PhraseRefer 和 ScanEnts 等工作同样说明了锚点信息对三维定位的重要性，但它们依赖额外的人工短语或锚点标注。CoT3DRef 的区别在于，通过规则解析、场景图解析和 GPT-3.5 生成伪标签，尝试在不增加人工锚点标注的情况下构造这一中间监督。
...
Visual Encoder 对每个候选物体的点云进行编码，得到视觉特征。设场景中有 $L$ 个候选物体，则视觉特征可以表示为：

$$
V = \{V_k\}_{k=1}^{L}, \qquad V_k \in \mathbb{R}^{1 \times d}
$$

模型后续主要在这些候选物体之间进行目标选择和位置细化，因此 CoT3DRef 并不是一个完全从原始点云开始的端到端物体检测系统。候选物体质量是它能够定位目标的前提条件。

### 3.2 自然语言编码

Language Encoder 使用预训练 BERT 对输入描述进行编码，得到语言特征。对于示例描述，语言侧需要识别出以下对象：
...
$$
F = \{F_k\}_{k=1}^{L}, \qquad F_k \in \mathbb{R}^{1 \times d}
$$

CoT3DRef 对融合模块的具体形式没有做强约束，因此可以集成到使用图网络、Transformer 或多视角特征的已有架构中。
...
在这一阶段需要区分两类输出：语言侧的对象预测回答“描述中出现了哪些对象”，视觉侧的 referring head 才回答“这些对象分别对应场景中的哪些候选实例”。对象类别识别和实例定位并不是同一个任务。
...
从实现角度看，CoT Decoder 的查询可以写成：

$$
Q = R^F + O^P
$$

其中 $R^F$ 提供并行阶段得到的初始位置，$O^P$ 表示经过 Pathway Head 排序后的对象序列。解码器以融合特征 $F$ 作为键和值，其注意力计算形式为：

$$
A = \operatorname{Softmax}\left(\frac{(R^F + O^P)F^\top}{\sqrt{d}}\right)
$$

因此，$O^P$ 的作用不是提供三维坐标，而是告诉解码器当前查询对应定位链中的哪一个对象以及哪一个逻辑步次。

论文还比较了 causal 和 non-causal 两种掩码方式。在 MVT 上，Nr3D 的准确率分别为 60.4 和 60.9，Sr3D 均为 73.2。该结果表明，严格的因果掩码并不是性能提升的唯一来源，显式对象路径、锚点监督和链式解码的组合才是主要因素。
...
$$
\mathcal{L}
= \lambda_V\mathcal{L}_{cls}^{V}
+ \lambda_T\mathcal{L}_{cls}^{T}
+ \lambda_{ref}\left(\mathcal{L}_{ref}^{P}+\mathcal{L}_{ref}^{CoT}\right)
+ \lambda_{dist}\mathcal{L}_{dist}
$$

其中：

- $\mathcal{L}_{cls}^{V}$：视觉侧候选物体的类别分类损失；
- $\mathcal{L}_{cls}^{T}$：语言侧目标和锚点类别预测损失；
- $\mathcal{L}_{ref}^{P}$：并行定位阶段的目标和锚点定位损失；
- $\mathcal{L}_{ref}^{CoT}$：CoT Decoder 输出的链式定位损失；
- $\mathcal{L}_{dist}$：区分目标与同类别干扰物的辅助损失。
...
论文采用较为轻量的 Transformer 配置：

- Pathway 模块使用 1 层 Transformer encoder；
- CoT Decoder 使用 1 层 Transformer decoder；
- Pathway 模块和 CoT Decoder 的注意力头数分别为 7 和 16；
- 候选物体数量 $L$ 设置为 52，最大句子长度 $W$ 设置为 24；
- 最大对象序列长度 $M$ 在 Nr3D 和 Sr3D 上分别设置为 8 和 3；
- 每个候选物体随机采样 1024 个点，隐藏维度 $d$ 设置为 768；
- 使用 Adam 优化器训练 100 个 epoch，初始学习率为 $10^{-4}$。

作者还比较了 CoT Decoder 使用 1、2 和 4 个 Transformer block 的结果。性能差异很小，因此最终采用单层 decoder。这说明论文的主要收益来自任务分解和对象路径建模，而不是简单堆叠更深的解码器。
...
- **论文主页：** [CoT3DRef Project Page](https://eslambakr.github.io/cot3dref.github.io/)
- **论文摘要：** [arXiv:2310.06214](https://arxiv.org/abs/2310.06214)
- **论文 PDF：** [Download PDF](https://arxiv.org/pdf/2310.06214)
- **代码仓库：** [CoT3D_VG](https://github.com/eslambakr/CoT3D_VG)
